...
export class WsService {
    ...
    readonly TkRdsName = "wsToken";
    readonly LfRdsName = "wsLife";
    readonly RpRdsName = "wsReport";
    ...
    constructor(
        ...
        this.redisService.client.del(this.TkRdsName);
        this.redisService.client.del(this.LfRdsName);
        this.redisService.client.del(this.RpRdsName);

        // Regular check is_alive
        this.lifeCheckTimer = setInterval(async () => {
            const lifes = await this.redisService.client.hgetall(
                this.LfRdsName
            );
            for (const wsId in lifes) {
                if (lifes[wsId] == "1") {
        ...
        this.clearTokenTimer = setInterval(async () => {
            const tokens = await this.redisService.hgetallObj(
                this.TkRdsName,
                Token
            );
            for (const wsId in tokens) {
                const tokenItem = tokens[wsId];
                if (
                    ...
                    await this.redisService.client.hdel(this.TkRdsName, wsId);
                    await this.redisService.client.hdel(this.LfRdsName, wsId);
                    await this.redisService.client.hdel(this.RpRdsName, wsId);
    ...
    // }
    // return ret;
    async getLifeMap(): Promise<Record<string, string>> {
        return this.redisService.client.hgetall(this.LfRdsName);
    }
    async getTokenMap(): Promise<Record<string, Token>> {
        return this.redisService.hgetallObj(this.TkRdsName, Token);
    }
    async getReportMap(): Promise<Record<string, StatusReportPayload>> {
        return this.redisService.hgetallObj(
            this.RpRdsName,
            StatusReportPayload
        );
    }
    ...
    async getToken(
        ...
        await this.redisService.hsetObj(this.TkRdsName, token, {
    ...
    async checkToken(token: string, req: IncomingMessage): Promise<boolean> {
        ...
        const tokenItem = await this.redisService.hgetObj(
            "wsToken",
            token,
            Token
        );
    ...
    private async getSolvePing(
        ...
        return async (data: Buffer): Promise<void> => {
            await this.redisService.client.hset(this.LfRdsName, wsId, "1");
            let report: StatusReportMessage;
            try {
                report = JSON.parse(data.toString("utf-8"));
            } catch (error) {
                const e = `${wsId.split(".")[0]}：解析 ping 的 JSON 时出错`;
                this.logger.error(e);
                throw new InternalServerErrorException(e);
            }
            if (report.type != MessageType.StatusReport) {
            ...
            await this.redisService.hsetObj(this.RpRdsName, wsId, report.body);
        };
    }
    ...
    private async updateTokenStatus(
        ...
        // 以下仔细处理，避免 bug
        // 务必第一时间修改 redis 中 token 的状态！再通知其他模块！
        // 添加心跳检测、通知评测机池
        if (status == TokenStatus.Unused) {
            ...
            tokenItem.msg = msg;
            await this.redisService.hsetObj(this.TkRdsName, wsId, tokenItem);
